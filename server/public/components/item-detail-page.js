    ...
    return html`
      ...
      <div class="mt-4 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white lg:col-span-1">
          <img src="${this.item.image || ''}" alt="${this.item.name}" class="h-full w-full object-cover" />
        </div>
      ...
      ${this.currentUser
        ? this.chatOtherUserId
          ...
          : html`
            ...
            `
        : html`
            <section class="mt-8 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 p-8 text-center">
              ...
              <h3 class="mt-4 text-sm font-semibold text-slate-900">Want to buy this?</h3>
              <p class="mt-2 text-sm text-slate-600">Please <a href="/login" class="font-bold text-indigo-600 hover:underline">login</a> to negotiate price and chat with the seller.</p>
              <div class="mt-6">
                <a href="/login" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700">Login Now</a>
              </div>
            </section>
          `}
    `;
